...
public class lts {
    ...
    private void handleConnection(Socket socket) throws IOException {
        ...
        if (!keepAlive) {
            ...
            handleWithKeepAlive(socket); // Persistent connection chosen for keep-alive mode
        }
    }
    ...
    private void handleBasic(Socket socket) throws IOException {
        ...
        int status = 500; // Represents the server's worst-case scenario: an unhandled internal error
        String method = "N/A", path = "N/A";
        ...
        OutputStream out = socket.getOutputStream()) {
            ...
            if (requestLine == null || requestLine.isEmpty()) {
                return;
            } // Close connection if there is no valid request

            String[] validatedRequest = validateRequest(requestLine);
            if (validatedRequest == null) {
                status = 400; 
                sendError(out, status, "Bad Request", shouldKeepAlive);
                return;
            }

            method = validatedRequest[0];
            path = validatedRequest[1];

            // Read headers as a pair of header's name: value
            Map<String, String> headers = parseHeaders(in);

            if (!method.equals("GET")) {
                status = 405; // status code for method not allowed
                sendError(out, status, "Method not allowed", shouldKeepAlive);
                return;
            }

            dispatchRequest(out, path, shouldKeepAlive); // dispatch request to HandleBasic
            status = 200;
        } catch (HttpStatusException e) {
            status = e.getStatusCode();
        } catch (IOException e) {
            status = 500; // Common for client disconnect during transmission
            if (!quiet) System.err.println("I/O error during handleBasic: " + e.getMessage());
        } finally {
            if (!quiet) {
                long elapsedTime = System.currentTimeMillis() - startTime;
                System.out.printf("%s %s %d %dms\n", method, path, status, elapsedTime);
            }
        }
    }
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            ...
            while (true) { 
                ...
                try {
                    ...
                    String connection = headers.getOrDefault("connection", "").trim();

                    boolean close = connection.equalsIgnoreCase("close");
                    shouldKeepAlive = !close;
                    ...
                    shouldKeepAlive = false;
                    if (!quiet) {
                        System.err.println("Connection timeout (Persistent connection expired).");
                    }
                    break;
                } catch (HttpStatusException e) {
                    status = e.getStatusCode();
                    shouldKeepAlive = false;
                } catch (Exception e) { // Catch all other unexpected exceptions (I/O, Runtime)
                    status = 500;
                    if (!quiet) {
                        System.err.println("Unexpected error occured during persistent request: " + e.getMessage());
                    } try {
                    ...
                    if (!quiet) {
                        long elapsedTime = System.currentTimeMillis() - startTime;
                        System.out.printf("%s %s %d %dms %s\n", method, path, status, elapsedTime, shouldKeepAlive ? "Keep-alive" : "Close"); 
                    }
                }
    ...
    private void handleConnectionThreaded(Socket socket) {
        ...
        Thread.ofVirtual().start(() -> {
            try {
                ...
                if (!quiet) {
                    System.err.println("I/O error in virtual thread handler: " + e.getMessage());
                } 
            } finally {
                try {
    ...
    private void handleEcho(OutputStream out, String path, boolean shouldKeepAlive) throws IOException {
        ...
        if (parts.length < 3 || parts[2].isEmpty()) {
            sendError(out, 400, "Bad Request: Missing size parameter.", shouldKeepAlive);
        ...
        try {
            ...
            sendError(out, 400, "Bad Request: Invalid size parameter.", shouldKeepAlive);
            throw new HttpStatusException(400);
        }

        if (size < 0) {
            sendError(out, 400, "Bad Request: Size must be non-negative.", shouldKeepAlive);
            throw new HttpStatusException(400);
        }
        ...
        try {
            ...
            sendError(out, 500, "Internal Server Error: SHA-256 algorithm not found", shouldKeepAlive);
    ...
    private void handleStaticFile(OutputStream out, String path, boolean shouldKeepAlive) throws IOException {
        ...
        if (path.contains("..")) {
            // Try to send 403 error before failing
            try {
                sendError(out, 403, "Forbidden: Directory traversal attempt detected.", shouldKeepAlive);
            } catch (IOException e) {
                // If client disconnects while sending error
            }
        ...
        Path absolutePublicDir = Paths.get(PUBLIC_DIR).toAbsolutePath().normalize();
        Path absoluteFilePath  = absolutePublicDir.resolve(rel).normalize();


        if (!absoluteFilePath.startsWith(absolutePublicDir)) {
            try {
                sendError(out, 403, "Forbidden: Security boundary violation detected.", shouldKeepAlive);
            } catch (IOException e) {
                // If client didconnects while sending error
            }
            throw new HttpStatusException(403);
        }

        if (!Files.exists(absoluteFilePath) || !Files.isRegularFile(absoluteFilePath)) {
            // Try customed 404 from phase 2 or the default 404 
            if (!tryServeCustom404(out, shouldKeepAlive)) {
                sendError(out, 404, "File not found.", shouldKeepAlive);
            }
            throw new HttpStatusException(404);
        }

        try {
            byte[] content = Files.readAllBytes(absoluteFilePath);
            String contentType = guessContentType(absoluteFilePath.toString());

            sendResponse(out, 200, "OK", contentType, content, null, shouldKeepAlive);
        } catch (IOException e) {
            sendError(out, 500, "Internal Server Error: Could not read file.", shouldKeepAlive);
            throw new HttpStatusException(500);
        }
    }
    ...
    private boolean tryServeCustom404(OutputStream out, boolean shouldKeepAlive) throws IOException {
        ...
        Path customed404 = Paths.get(PUBLIC_DIR, "404.html");

        if(Files.exists(customed404) && Files.isRegularFile(customed404)) {
            try {
                byte[] content = Files.readAllBytes(customed404);

                sendResponse(out, 404, "Not Found", "text/html", content, null, shouldKeepAlive);
                return true;
            } catch (IOException e) {
                System.err.println("Error reading or sending custom 404 page: " + e.getMessage());
                return false;
            }
        } else {
            return false;
    ...
    private void sendResponse(OutputStream out, int code, String message, String contentType,
        ...
        writer.print("HTTP/1.1 " + code + " " + message + CRLF);
        writer.print("Content-Type: " + contentType + CRLF);
        writer.print("Content-Length: " + body.length + CRLF);
        writer.print("Connection: " + connectionHeader + CRLF);

        // Write extra headers (if implemented)
        if (extraHeaders != null) {
            for (Map.Entry<String, String> header : extraHeaders.entrySet()) {
                writer.print(header.getKey() + ": " + header.getValue() + CRLF);
            }
        } 

        // Write blank line (separates headers from body)
        writer.print(CRLF);

        // Flush the PrintWriter (sends headers)
        writer.flush(); 

        // Write body bytes directly to OutputStream
        out.write(body);

        // Flush the OutputStream (sends body)
        out.flush();
    }
    ...
    private void sendError(OutputStream out, int code, String message, boolean shouldKeepAlive)
        ...
        String body = String.format("<html><body><h1>%d %s</h1></body></html>", code, message);
    ...
    // Custom exception to pass the final HTTP status code (e.g., 403, 404, 500) 
    // from a deep request handler like handleStaticFile up to the request loop
    // like handleBasic or handleWithKeepAlive.
